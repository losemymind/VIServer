...
#ifndef LOSEMYMIND_IPV4ADDRESS_H
#define LOSEMYMIND_IPV4ADDRESS_H

#pragma once
#include <string>
#include "FoundationKit/GenericPlatformMacros.h"
#include "FoundationKit/Foundation/ByteSwap.h"
...
public:
	 ...
	 */
	IPv4Address(uint8 InA, uint8 InB, uint8 InC, uint8 InD)
#if (ENDIANNESS_ORDER == ENDIANNESS_LITTLE_ENDIAN)
		: D(InD)
		, C(InC)
		, B(InB)
		...
		, C(InC)
		, D(InD)
#endif // (ENDIANNESS_ORDER == ENDIANNESS_LITTLE_ENDIAN)
	{ }
...
public:
	 ...
	 * @see ToString
	 */
    static  bool Parse(const std::string& AddressString, IPv4Address& OutAddress)
    {
        std::vector<std::string> Tokens = StringUtils::split(AddressString, '.');
        if (Tokens.size() == 4)
        {
            OutAddress.A = std::stoi(Tokens[0].c_str());
            OutAddress.B = std::stoi(Tokens[1].c_str());
            OutAddress.C = std::stoi(Tokens[2].c_str());
            OutAddress.D = std::stoi(Tokens[3].c_str());
            return true;
        }
        return false;
    }
...
public:
	...
	static const IPv4Address InternalLoopback;

	/** Defines the broadcast address for the 'zero network' (i.e. LAN), which is 255.255.255.255. */
	static const IPv4Address LanBroadcast;
};




#endif // LOSEMYMIND_IPV4ADDRESS_H
